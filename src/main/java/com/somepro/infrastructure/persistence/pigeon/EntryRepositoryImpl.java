 ...
 * 集鸽登记仓储适配器。
 *
 * 写入并发兜底：应用层「先查 (race_id, band_id) 后插」之间可能被另一笔抢先，
 * t_entry.uk_race_band 在库侧兜底，撞键翻译成业务异常（同一羽报同一场只登一次）。
 */
@Repository
public class EntryRepositoryImpl extends PigeonBlockingRepository implements EntryRepository {
    ...
    public Mono<Entry> save(Entry entry) {
        return blocking(() -> {
            ...
            try {
                ...
                throw new BizException("该羽赛鸽本场赛已集过鸽，重复登记无效");
            }
    ...
    private static EntryRow toRow(EntryRowPO po) {
        return new EntryRow(po.getEntryId(), po.getBandId(), po.getBandCode(),
                po.getOwnerName(), po.getBasketNo(), po.getEntryTime());
    }
}
